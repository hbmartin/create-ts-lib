...
**Nothing here formats the template assets, so the scaffolded project is the first thing that does.** oxfmt ignores JSON/JSONC/YAML/Markdown, Biome excludes `source/templates/assets`, and `.tmpl` keeps the rest out of both — so an asset that the formatter *we chose for the user* would reprint sails through this repo's `check` and fails theirs on the first run. Biome collapses a short JSON array onto one line and always expands `package.json`, while `JSON.stringify` always breaks arrays across lines: that mismatch shipped `.fallowrc.jsonc` and `jsr.json` broken to every Biome project. Generated manifests other than `package.json` build short arrays through `inlineJsonArray` (`source/templates/render.ts`), and `test/generated-formatting.test.ts` runs the real formatters over the real rendered output — both toolings, minimal and every-feature answers, with a planted misformatted file as the control. Adding a JSON, JSONC, or TypeScript asset means it is covered there; the CI smoke matrix is not a substitute, since it exercises only a few answer combinations and only after an install.
